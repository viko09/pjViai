...
# Load data from file
data = np.loadtxt('cross.csv', delimiter=',', skiprows=1)
...
# Plot values
plt.plot(time, color, color='orangered', alpha=0.65)
plt.xlabel('Time')
plt.ylabel('Color')
plt.title('Color Values over Time')
plt.grid()
plt.show()
...
note_duration = 0.3600  # in seconds
...
# Save MIDI file
midi_file.save('wizard.mid')

# Play MIDI file using timidity
command = 'timidity -Ow -A120 -EFreverb=0.8 -EFchorus=0.8,0.7,130 -EFflanger=0.8,0.65,0.5 wizard.mid -o - | ffmpeg -i '\
          '- -acodec libmp3lame -ab 64k wizardio.mp3'
