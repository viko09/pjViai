...
np.savetxt('NewLux.csv', data, delimiter=',', header='C1,C2', comments='', fmt='%.3f')

os.system('python3 midiMaker.py')
...
line, = ax.plot([center_x, center_x], [center_y, center_y - 50], color='green',
...
ani.save('swept1.mp4', writer=writer)

# add audio to the mp4 file
os.system('ffmpeg -i lux_ghostlyTst.mp3 lux_ghostlyTst.wav')
os.system('sox lux_ghostlyTst.wav -n spectrogram -o sonogramTst.png')
os.system('geeqie sonogram.png')
os.system('ffmpeg -i lux_ghostlyTst.mp3 -i swept.mp4 -map 0:a -map 1:v -c copy -shortest swept_with_audio.mp4')
